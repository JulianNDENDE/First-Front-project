<template>
    <div class="w-full relative z-50 onlysearch">
      <div
        class="flex flex-row items-center bg-white relative border-2 border-black overflow-hidden rounded-md justify-between h-16 onlysearch">
        
        <div class="flex flex-row items-start items-center relative border-r-2 border-black onlysearch">
          <span class="px-3 onlysearch">
            <svg xmlns="http://www.w3.org/2000/svg" class="onlysearch" width="1.5em" height="1.5em" viewBox="0 0 512 512"><title>Search</title><path d="M221.09 64a157.09 157.09 0 10157.09 157.09A157.1 157.1 0 00221.09 64z" fill="none" stroke="currentColor" stroke-miterlimit="10" stroke-width="32"/><path fill="none" stroke="currentColor" stroke-linecap="round" stroke-miterlimit="10" stroke-width="32" d="M338.29 338.29L448 448"/></svg>
          </span>
          <input
            type="text"
            class="text-xl pl-1 w-3/4 py-3 focus:outline-none onlysearch"
            placeholder="Dépression, anxiété..."
            v-model="search">
          <a v-if="search != ''" 
             @click="search = ''"
             class="absolute right-3 text-2xl cursor-pointer onlysearch">
                x</a>
        </div>

        <div class="flex flex-row items-start items-center relative onlysearch">
          <span class="px-3 onlysearch">
              <svg class="fill-gray-400 onlysearch" xmlns="http://www.w3.org/2000/svg" width="1.5em" height="1.5em" viewBox="0 0 512 512"><path d="M448 64L64 240.14h200a8 8 0 018 8V448z" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="32"/></svg>
          </span>

          <input
            type="text"
            ref="streetRef"
            class="w-2/3 text-xl px-2 focus:outline-none onlysearch"
            placeholder="Paris, Lyon..."
            v-model="city"
            >
            <div class="hidden lg:flex onlysearch">
              <label @click="findMyCity"><svg xmlns="http://www.w3.org/2000/svg" class="fill-gray-400 stroke-gray-400 onlysearch" width="2em" height="2em" viewBox="0 0 512 512"><title>Compass</title><path d="M448 256c0-106-86-192-192-192S64 150 64 256s86 192 192 192 192-86 192-192z" fill="none" stroke-miterlimit="10" stroke-width="32"/><path d="M350.67 150.93l-117.2 46.88a64 64 0 00-35.66 35.66l-46.88 117.2a8 8 0 0010.4 10.4l117.2-46.88a64 64 0 0035.66-35.66l46.88-117.2a8 8 0 00-10.4-10.4zM256 280a24 24 0 1124-24 24 24 0 01-24 24z"/></svg></label>
      ...
      class="absolute right-3 text-sm pt-2 link link-hover onlysearch">Recherche avancée</div>

      <!-- résultat de la recherche sur les conditions --> 
      <div
        v-if="newconditions.length > 0"
        class="w-full absolute z-50 bg-white border-2 border-gray-300 mt-1 max-height-48 overflow-hidden overflow-y-scroll rounded-md shadow-md onlysearch"
        >
        <div class="px-3 py-2 cursor-pointer hover:bg-gray-100 onlysearch"
            v-for="condition in newconditions">
            <div class="flex flex-row justify-between px-5 onlysearch">
                <span @click="selectResult(condition)" @mousedown.prevent>
                  {{ condition.groupname }}
                </span>
                <span class="badge badge-outline text-medium onlysearch">
